...
43:51 - 48:17 // how to determine the downtime required for rebuilding indexes for huge database tables?
    ...
    - Best case: use Ola Hallengren script. It has a Commandlog table, which logs timings, or if there is no history, we can go with the Reorg index.
...
Note:
----
Below activities can be queried to check the progress of completion status.
